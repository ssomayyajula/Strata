...
outputs (from f's declared signature) via effectfulCall and checks the
continuation at the residual grade:

```
Γ ⊢_p f(V₁,...,Vₙ) ⇒ B & d    subsume(B, A) = c
Γ,x₁:T₁,...,xₖ:Tₖ ⊢_p K ⇐ A & (d\e)
────────────────────────────────────────────────────────────────────────────
Γ ⊢_p effectfulCall f [V₁,...,Vₙ] [x₁:T₁,...,xₖ:Tₖ] (c(xⱼ); K) ⇐ A & e
```
...
`labeledBlock` introduces l into scope for M_b (intro rule).
`exit l` uses l from scope (elim rule). Both are non-returning — exit
jumps to the block's after-continuation K, unit delegates to the
...
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
⟦D⟧⇐ₚ :: ⟦Γ⟧,x₁:T₁,...,xₖ:Tₖ ⊢_p effectfulCall g [V₁,...,Vₙ] [x₁:T₁,...,xₖ:Tₖ] M_k ⇐ ⟦A⟧ & e
```

#### Clauses of ⟦·⟧⇐ₚ

All clauses receive ambient grade e = procGrades[f] (or a residual from
an enclosing effectfulCall).
...
⟦D₁⟧⇐ᵥ :: ⟦Γ⟧ ⊢_v V₁ ⇐ ⟦A₁⟧  ...  ⟦Dₙ⟧⇐ᵥ :: ⟦Γ⟧ ⊢_v Vₙ ⇐ ⟦Aₙ⟧    ⟦K'⟧⇐ₚ :: ⟦Γ⟧,x₁:T₁,...,xₖ:Tₖ ⊢_p (assign x (subsume(xⱼ, ⟦Γ(x)⟧)) M_k) ⇐ ⟦A⟧ & (d\e)
──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
⟦D⟧⇐ₚ :: ⟦Γ⟧,x₁:T₁,...,xₖ:Tₖ ⊢_p effectfulCall g [V₁,...,Vₙ] [x₁:T₁,...,xₖ:Tₖ] (assign x (subsume(xⱼ, ⟦Γ(x)⟧)) M_k) ⇐ ⟦A⟧ & e
