  ...
  | .Compare ann left _ comparators => match ann.info with
    | .funcCall sig => do
        if comparators.val.size != 1 then
          -- Chained comparisons: lower a < b < c to (a < b) && (b < c)
          -- For now emit a hole (sound under-approximation)
          mkExpr sr .Hole
        else do
  ...
  let procedures ← if otherStmts.isEmpty then pure procedures
      ...
      let mainOutputs : List Parameter :=
        [{ name := rtLaurelResult, type := mkTypeDefault (.TCore "Any") },
         { name := rtMaybeExcept, type := mkTypeDefault (.TCore "Error") }]
      let mainProc : Procedure := { name := rt "__main__", inputs := [], outputs := mainOutputs, preconditions := [], decreases := none, isFunctional := false, body := .Opaque [] (some bodyBlock) [] }
