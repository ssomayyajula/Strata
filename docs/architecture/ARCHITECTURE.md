...
Note: `unit` checks at any `A & e` (axiom) — it is the trivial producer
that delegates to the enclosing continuation. `exit l` synthesizes by
looking up label `l` in the context.
...
There is an induced translation on types ⟦A⟧ = eraseType(A) and on
contexts ⟦Γ⟧ = { (x : ⟦A⟧) | (x : A) ∈ Γ }.

```
⟦·⟧⇒ᵥ : (D :: Γ ⊢_L e : A)        → (⟦D⟧⇒ᵥ :: ⟦Γ⟧ ⊢_v V ⇒ ⟦A⟧)
⟦·⟧⇐ᵥ : (D :: Γ ⊢_L e : A, B)     → (⟦D⟧⇐ᵥ :: ⟦Γ⟧ ⊢_v V ⇐ B)
⟦·⟧⇒ₚ : (D :: Γ ⊢_L e : A)        → SynthResult
⟦·⟧⇐ₚ : (D :: Γ ⊢_L S;rest : A, e) → (⟦D⟧⇐ₚ :: ⟦Γ⟧ ⊢_p M ⇐ ⟦A⟧ & e)
```

Each output is a valid derivation in the GFGL system. Mode correctness
is auditable by checking each output against the GFGL rules above.
...
If grade(f) = d > pure: ⟦D⟧⇒ₚ = .call f [⟦D₁⟧⇐ᵥ,...,⟦Dₙ⟧⇐ᵥ] ⟦B⟧ d
```

#### Producer subsumption in ⟦·⟧
...
⟦D⟧⇐ₚ :: ⟦Γ⟧ ⊢_p exit l ⇐ ⟦A⟧ & e     (via producer synthesis + subsumption)
...
(apply producer subsumption — see above)
...
(apply producer subsumption with continuation: assign x (subsume(r, ⟦Γ(x)⟧)) ⟦K⟧⇐ₚ)
...
D_e :: Γ ⊢_L e : B    K :: Γ ⊢_L rest : A
────────────────────────────────────────────
D :: Γ ⊢_L e; rest : A    (expr-as-stmt)    where ⟦D_e⟧⇒ₚ = .call f args B d

        ↦

(apply producer subsumption — result discarded)
```
...
⟦Dᵢ_args⟧⇐ᵥ :: ⟦Γ⟧ ⊢_v Wⱼ ⇐ ...    ⟦cont⟧⇐ₚ :: ⟦Γ⟧,rᵢ:Bᵢ ⊢_p ... ⇐ ⟦A⟧ & (dᵢ\e)
──────────────────────────────────────────────────────────────────────────────────────────
⟦Γ⟧,rᵢ:Bᵢ ⊢_p effectfulCall gᵢ argsᵢ [rᵢ:Bᵢ] ⟦cont⟧⇐ₚ ⇐ ⟦A⟧ & e
```

Bound variable rᵢ replaces eᵢ in the outer call's argument list.
